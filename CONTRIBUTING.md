...
- **One home per concept.** Helpers more than one domain needs live once, in `utils/core/`, and no public function name is defined twice in `utils/`. A domain too big for one readable file becomes a package of small modules named for what they do (`utils/assets/`, `utils/physics/`, `utils/scatter/`, `utils/variants/`); services call it as `<package>.<module>.<function>` (`physics.apis.apply_api`), the way they call `camera_utils.create_camera`. Domains import core; core never imports a domain (only the standard library, `pxr`, `numpy`, `bowerbot.schemas` and other core modules). Bounding boxes always come from `core/bounds.py`.
